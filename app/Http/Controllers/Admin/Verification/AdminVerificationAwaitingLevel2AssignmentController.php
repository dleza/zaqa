...
use App\Http\Requests\Admin\Verification\BulkAssignAwaitingLevel2Request;
...
class AdminVerificationAwaitingLevel2AssignmentController extends Controller
{
    ...
    public function index(Request $request, QualificationsPoolService $pool): Response
    {
        ...
        $rows = $pool->awaitingLevel2Assignment($request);
    ...
    public function bulkAssign(
        BulkAssignAwaitingLevel2Request $request,
        VerificationAssignmentBulkService $bulk,
    ): RedirectResponse {
        return $this->bulkAssignFromQueue(
            $request,
            fn (User $actor, User $assignee, array $ids, ?string $comment) => $bulk->bulkAssignLevel2($actor, $assignee, $ids, $comment),
        );
    }
