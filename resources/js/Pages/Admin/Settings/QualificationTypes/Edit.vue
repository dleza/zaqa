...
const form = useForm({
  zqf_level_code: props.type.zqf_level_code,
  level_label: props.type.level_label,
  name: props.type.name,
  short_name: props.type.short_name ?? '',
  description: props.type.description ?? '',
  billing_category_id: props.type.billing_category_id,
  requires_subject_results: !!props.type.requires_subject_results,
  is_active: !!props.type.is_active,
  sort_order: props.type.sort_order ?? 0,
})

function submit() {
  form.put(`/admin/settings/qualification-types/${props.type.id}`, { preserveScroll: true })
}
</script>
      ...
      <template #header>
        <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <h1 class="mt-2 text-2xl font-semibold tracking-tight text-text-primary">Edit qualification type</h1>
            <p class="mt-1 text-sm text-text-muted">Update qualification type details.</p>
          </div>
