...
const props = defineProps<{
  title: {
    ...
    is_active: boolean
    sort_order: number
    awarding_institution_ids: number[]
    usage_count: number
  }
...
const form = useForm({
  ...
  qualification_type_id: props.title.qualification_type_id ?? '',
  description: props.title.description ?? '',
  is_active: props.title.is_active,
  sort_order: props.title.sort_order,
  awarding_institution_ids: [...props.title.awarding_institution_ids],
})
...
function submit() {
  form.put(`/admin/settings/qualification-titles/${props.title.id}`, { preserveScroll: true })
}
</script>
      ...
      <template #header>
        <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <h1 class="mt-2 text-2xl font-semibold tracking-tight text-text-primary">Edit qualification title</h1>
            <p class="mt-1 text-sm text-text-muted">Used on {{ title.usage_count }} qualification(s). Historical snapshots on applications are not changed automatically.</p>
          </div>
