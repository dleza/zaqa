...
const form = useForm({
  ...
  code: '',
  description: '',
  local_processing_days: null as number | null,
  foreign_processing_days: null as number | null,
  is_active: true,
  sort_order: 0,
})

function submit() {
  form.post('/admin/settings/billing-categories', { preserveScroll: true })
}
</script>
      ...
      <template #header>
        <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <h1 class="mt-2 text-2xl font-semibold tracking-tight text-text-primary">Add billing category</h1>
            <p class="mt-1 text-sm text-text-muted">Create a category, then add fee structures and map qualification types to it.</p>
          </div>
      ...
      <div class="rounded-2xl border border-border bg-surface p-6 shadow-sm">
        <form class="space-y-5" @submit.prevent="submit">
          ...
          <div>
            ...
            <input v-model="form.code" class="zaqa-input font-mono uppercase" autocomplete="off" placeholder="e.g. LOCAL_PROFESSIONAL" />
            <div class="mt-1 text-xs text-text-muted">Stable identifier used internally. Cannot be changed after creation.</div>
