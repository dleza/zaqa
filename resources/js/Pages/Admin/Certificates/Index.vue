...
import { BadgeCheck, ExternalLink, FileDown, FileSpreadsheet, Search } from 'lucide-vue-next'
      ...
      <div class="border-b border-border bg-surface-muted px-5 py-4">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <div class="mt-1 text-xs text-text-muted">Includes superseded rows after reissue (status “reissued”).</div>
          </div>
        ...
        <table class="min-w-full text-sm">
          ...
          <tbody class="divide-y divide-border/60">
            <tr v-for="row in certificates.data" :key="row.id" class="hover:bg-surface-muted/60">
              ...
              <td class="px-5 py-3">
                <span class="zaqa-badge capitalize" :class="statusBadgeClass(row.status)">{{ row.status }}</span>
              </td>
