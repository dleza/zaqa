      ...
      <template #header>
        <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <h1 class="mt-2 text-3xl font-semibold tracking-tight text-text-primary">Create role</h1>
            <p class="mt-1 text-sm text-text-muted">Define a role name and assign permissions grouped by module.</p>
          </div>
            ...
            <section class="rounded-3xl border border-border/70 bg-surface p-6 shadow-[0_20px_50px_-34px_rgba(15,23,42,0.38)]">
              ...
              <div class="mt-5 space-y-5">
                <div>
                  ...
                  <input v-model="form.name" class="zaqa-input mt-2" type="text" autocomplete="off" placeholder="e.g. Finance Supervisor" />
                  <div class="mt-2 text-xs text-text-muted">Choose a short, clear name that staff can recognize quickly.</div>
                ...
                <div class="rounded-2xl border border-border/70 bg-surface-muted/35 px-4 py-4 text-sm text-text-muted">
                  Include <span class="font-semibold text-text-primary">dashboard.view</span> for roles that must access the admin portal.
                </div>
              </div>
            </section>
            ...
            <section class="rounded-3xl border border-border/70 bg-surface p-6 shadow-[0_20px_50px_-34px_rgba(15,23,42,0.38)]">
              <div class="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div class="mt-1 text-xs text-text-muted">Keep permission assignment structured and minimal.</div>
                </div>
              </div>
          ...
          <section class="min-w-0 rounded-3xl border border-border/70 bg-surface p-6 shadow-[0_20px_50px_-34px_rgba(15,23,42,0.38)] sm:p-7">
            <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
              <div>
                ...
                <h2 class="mt-2 text-lg font-semibold text-text-primary">Assign permissions by module</h2>
                <p class="mt-1 text-sm text-text-muted">Search, expand modules, then select only the permissions this role actually needs.</p>
              </div>
        ...
        <div class="sticky bottom-4 z-10 pt-1">
          <div class="flex w-full flex-col gap-3 rounded-2xl border border-border/70 bg-surface/95 px-4 py-4 shadow-[0_18px_44px_-26px_rgba(15,23,42,0.38)] backdrop-blur sm:flex-row sm:items-center sm:justify-between sm:px-5">
            <div class="text-sm text-text-muted">
              <span class="font-semibold text-text-primary">{{ form.permissions.length }}</span> permissions selected for this role.
            </div>
