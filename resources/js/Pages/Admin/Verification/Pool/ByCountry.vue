...
import { Globe2, Search, ShieldCheck } from 'lucide-vue-next'
import { ref, watch } from 'vue'

const props = defineProps<{
  groups: Array<{ country_id: number | null; country_name: string; count: number }>
  filters?: {
    hide_zambia?: string | null
...
const hideZambia = ref<boolean>((props.filters?.hide_zambia ?? '') === '1')
...
watch([hideZambia, overdueDays, submittedFrom, submittedTo, qualificationQ], () => {
  router.get(
    '/admin/verification/pool/country',
    {
      hide_zambia: hideZambia.value ? '1' : null,
  ...
  <AdminLayout>
    <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
      <div>
        ...
        <h1 class="mt-2 text-2xl font-semibold tracking-tight text-text-primary">Category view: Country of award</h1>
        <p class="mt-1 text-sm text-text-muted">Applications grouped by country of award.</p>
      </div>
      <div class="flex items-center gap-2">
        <label class="inline-flex items-center gap-2 rounded-xl border border-border bg-surface px-3 py-2 text-sm text-text-primary">
          <input v-model="hideZambia" type="checkbox" class="h-4 w-4" />
          Hide Zambia
        </label>
    ...
    <div class="mt-6 overflow-hidden rounded-2xl border border-border bg-surface shadow-sm">
      <div class="border-b border-border bg-surface-muted px-5 py-4">
        <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <div class="text-sm font-semibold text-text-primary">Countries</div>
            <div class="mt-1 text-xs text-text-muted">Counts include submitted/resubmitted/in-progress applications.</div>
          </div>
      ...
      <div v-if="groups.length === 0" class="px-5 py-6 text-sm text-text-muted">No applications in the pool.</div>
      ...
      <div v-else class="divide-y divide-border/60">
        <Link
          ...
          :key="g.country_name"
          :href="
            `/admin/verification/pool?country_id=${g.country_id ?? ''}` +
        ...
        >
          <div class="flex items-center gap-3">
            ...
            <div class="text-sm font-semibold text-text-primary">{{ g.country_name }}</div>
          </div>
